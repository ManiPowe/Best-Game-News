...
if (!isset($_FILES['avatar']) || $_FILES['avatar']['error'] !== UPLOAD_ERR_OK) {
    die("Ошибка загрузки файла. Код ошибки: " . $_FILES['avatar']['error'] . " <a href='../..//cab'>Назад</a>");
}
...
if ($file_size > $max_size) {
    die("Файл слишком большой! Максимум 10 МБ. <a href='../..//cab'>Назад</a>");
}
...
if (!in_array($extension, $allowed_extensions)) {
    die("Разрешены только файлы форматов: JPG, PNG, WEBP, GIF. <a href='../..//cab'>Назад</a>");
}

$image_info = getimagesize($tmp_path);
if ($image_info === false) {
    die("Файл не является изображением! <a href='../..//cab'>Назад</a>");
}
...
if (!move_uploaded_file($tmp_path, $destination)) {
    die("Не удалось сохранить файл на сервере. <a href='../..//cab'>Назад</a>");
}
